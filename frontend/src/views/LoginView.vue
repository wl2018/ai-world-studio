<template>
  <div class="auth-page">
    <div class="auth-box card">
      <h1>🌐 {{ t('app.name') }}</h1>
      <p class="subtitle">{{ t('auth.login.title') }}</p>
      ...
      <form @submit.prevent="handleLogin">
        ...
        <div class="form-group">
          ...
          <input v-model="password" type="password" :placeholder="t('auth.fields.passwordPlaceholder')" required />
        </div>
      ...
      <p class="switch-link">
        ...
        <RouterLink to="/register">{{ t('auth.login.switchLink') }}</RouterLink>
      </p>
